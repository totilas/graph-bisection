...
filename = "./complet-"+str(num_hosts)+"-platform.xml"
fh = open(filename, 'w')
fh.write(issueHead())
...
for i in range(0,num_hosts):
	for j in range(i+1, num_hosts):
		fh.write(issueLink2(i,j))

# Create routes
for i in range(0, num_hosts):
	for j in range(i+1, num_hosts):
		fh.write(issueRouteHead(i,j))
		fh.write(issueRouteLink2(i,j))
		fh.write(issueRouteTail())


fh.write(issueTail())
fh.close()
print >> sys.stderr, "Complet graph XML platform file created: "+filename

###############################################################
## Generate host file
filename = "./complet-"+str(num_hosts)+"-hostfile.txt"
fh = open(filename, 'w')
